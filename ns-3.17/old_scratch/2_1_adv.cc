...
int main(int argc, char *argv[])
{
  ...
  NetDeviceContainer d1 = pointToPoint.Install(nodes.Get(0), nodes.Get(2));
  ...
  NetDeviceContainer d3 = pointToPoint.Install(nodes.Get(2), nodes.Get(4));
  address.SetBase("10.1.3.0", "255.255.255.0");
  Ipv4InterfaceContainer i3 = address.Assign(d3);
  printf(" interfaces.GetAddress(i) is %d\n\n",  i3.GetAddress(0).Get());

  NetDeviceContainer d4 = pointToPoint.Install(nodes.Get(3), nodes.Get(4));
