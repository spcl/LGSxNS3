...
int main(int argc, char *argv[])
{
  ...
  bool enable_qcn = false, use_dynamic_pfc_threshold = false, packet_level_ecmp = true, flow_level_ecmp = false;
  uint32_t packet_payload_size = 512, l2_chunk_size = 4000, l2_ack_interval = 2048;
  double pause_time = 0, simulator_stop_time = 3.01, app_start_time = 1.0, app_stop_time = 9.0;

  double cnp_interval = 50, alpha_resume_interval = 55, rp_timer, dctcp_gain = 1 / 16, np_sampling_interval = 0, pmax = 1;
  uint32_t byte_counter, fast_recovery_times = 55, kmax = 60, kmin = 60;
  ...
  NetDeviceContainer d5 = pointToPoint.Install(nodes.Get(4), nodes.Get(9));
  ...
  NetDeviceContainer d6 = pointToPoint.Install(nodes.Get(5), nodes.Get(9));
  ...
  NetDeviceContainer d7 = pointToPoint.Install(nodes.Get(6), nodes.Get(9));
  ...
  NetDeviceContainer d8 = pointToPoint.Install(nodes.Get(7), nodes.Get(9));
