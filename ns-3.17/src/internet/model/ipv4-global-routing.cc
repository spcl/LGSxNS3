...
Ipv4GlobalRouting::GetTupleValue (const Ipv4Header &header, Ptr<const Packet> ipPayload)
{
	...
	switch (header.GetProtocol ())
	{
        ...
        //printf("Node %d - Source Port %d - Dest Port %d - Source %d - Destination %d - Prot %d -- Actual %d\n",ns3::NodeList::GetNode(ns3::Simulator::GetContext())->GetId(), udpHeader.GetSourcePort(), udpHeader.GetDestinationPort (), header.GetSource ().Get (), header.GetDestination ().Get (), header.GetProtocol (), tupleValue);
				break;
			}
...
Ipv4GlobalRouting::LookupGlobal (const Ipv4Header &header, Ptr<const Packet> ipPayload, Ptr<NetDevice> oif)
{
  ...
  //printf("Routing on Node %d\n",ns3::NodeList::GetNode(ns3::Simulator::GetContext())->GetId());
  ...
  if (allRoutes.size () > 0 ) // if route(s) is found
    {
      ...
      if (m_randomEcmpRouting)
		...
		{
      //printf("Before using ECMP\n");
			selectIndex = GetTupleValue (header, ipPayload) % allRoutes.size ();
      //printf("Using ECMP, Size %d - Tuple %" PRIu32 " - Selected Index %d\n", allRoutes.size (), GetTupleValue (header, ipPayload), selectIndex );
		}
      else 
        {
          selectIndex = 0;
