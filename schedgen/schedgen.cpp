...
void create_ring_allreduces_rank(Goal *goal, int src_rank, int comm_size, int datasize, int chunk) {

  // First Phase AllRingReduce
  int tag = 1;
  int old_recv = -1;
  for (int i = 0; i < (comm_size - 1) * 2; i++) {
	int recv = goal->Recv(chunk,  modulo(src_rank-1, comm_size));	
	int send = goal->Send(chunk, (src_rank+1)%comm_size);

	goal->SetTag(tag++);

	if (i != 0) {
		goal->Requires(send, old_recv);
		goal->Requires(recv, old_recv);
		old_recv = recv;
	} else {
		old_recv = recv;
	}
...
void create_ring_allreduces(gengetopt_args_info *args_info) {
	...
	int chunk_to_send = datasize / comm_size;

	for (int src_rank = 0; src_rank < comm_size; src_rank++) {
		goal.StartRank(src_rank);
		create_ring_allreduces_rank(&goal, src_rank, comm_size, datasize, chunk_to_send);
		goal.EndRank();
	}
  goal.Write();
}
...
int main(int argc, char **argv) {
	...
	if (strcmp(args_info.ptrn_arg, "ring_allreduce") == 0) {
		 create_ring_allreduces(&args_info);
	}
